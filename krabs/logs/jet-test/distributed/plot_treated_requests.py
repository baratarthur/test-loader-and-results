...
def compute_metrics(requests):
    ...
    for second in sorted(grouped):
        ...
        for request in grouped[second]:
            ...
            db_start = int(request.get('databaseRequestStartTime', 0))
            db_end = int(request.get('databaseRequestEndTime', 0))
            ...
            metrics.append({
                ...
                'database_service': db_end - db_start,
                ...
                'wait_database': db_start - lookup_end if db_start != 0 and lookup_end != 0 else -1,
                'wait_cache_update': update_start - db_end if update_start != 0 and db_end != 0 else -1,
        ...
        wait_cache_update_data = [m['wait_cache_update'] for m in metrics if m['wait_cache_update'] != -1]
        wait_cache_update.append(mean(wait_cache_update_data) if len(wait_cache_update_data) > 0 else 0)
    ...
    return {
        ...
        'wait_cache_update': wait_cache_update,
...
def plot_metrics(metrics, output_path: Path | None = None):
    ...
    axes[1].plot(x, metrics['wait_cache_update'], label='Waiting before cache update', linewidth=1.5)
