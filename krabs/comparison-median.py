...
for file_path in csv_files:
    ...
    # 4. Identify the Mean/Average column (Locust uses 'Average Response Time')
    mean_col = None
    for col in ['Average Response Time', 'avg_response_time', 'avg', 'Average', 'Total Average Response Time']:
        if col in df_filtered.columns:
            mean_col = col
            break
            
    if mean_col is None:
        print(f"Error: Could not find Average Response Time column in '{file_path}'. Skipping.")
        continue
        
    # Clean label for the legend
    label_name = os.path.basename(file_path).replace('_stats_history.csv', '')
    
    # Plot the Mean Latency line on the main axis
    ax.plot(
...
ax.set_title('Locust Performance Test - Mean Latency Comparison', fontsize=14, fontweight='bold', pad=15)
...
output_image = f'results/{analysis_folder}/comparison/{method}/median.pdf'
